...
# Version: 0.0.5
# Timestamp: 2024-06-11 17:31:06 +0530
# Author: d disk
...
sub build_startupfile {
	...
	if ( defined $InspireExtras ) {
		my @inspireextras = split( ' ', $InspireExtras );
		foreach my $extras (@inspireextras) {
			if ( $extras =~ /(\w+)=(.+)/ ) {
				$startup_extras{$1} = $2;
			}
			else {
...
Development Options:
  ...
  --inspireextras	<INS_EXTRAS>	To add/update inspire extras parameter (optional)

  --startuponly                		build inspire INIFH file only; do not execute InspireRunJob
